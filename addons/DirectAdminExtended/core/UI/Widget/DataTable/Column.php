...
class Column
{
    ...
    public function __construct($name, $tableName = null, array $serchableName = null)
    {
        $this->name  = $name;
        $this->id    = $name;
        $this->class = '';
        ...
        if ($serchableName)
        {
            $this->searchableName = $serchableName;
        }
        else
        {
    ...
    public function setFilter(\ModulesGarden\DirectAdminExtended\Core\UI\Widget\DataTable\Filters $filter)
    {
        $this->filter = $filter;
    ...
    public function getFullNameList($wrapped = true)
    {
        $list = [];

        foreach ($this->searchableName as $name)
        {
            $vWrapp = $wrapped ? '`' : '';

            if ($this->tableName)
            {
                $list[] = $vWrapp . $this->tableName . $vWrapp . '.' . $vWrapp . $name . $vWrapp;
            }
            else
            {
                $list[] = $vWrapp . $name . $vWrapp;
            }
