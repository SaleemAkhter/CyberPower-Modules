...
use \ModulesGarden\DirectAdminExtended\Core\UI\Helpers\TemplateConstants;
...
trait Template
{
    ...
    protected function loadTemplateDir()
    {
        ...
        $base   = $isCore = null;
        if (str_contains($class, 'ModulesGarden\DirectAdminExtended\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\DirectAdminExtended\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\DirectAdminExtended\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\DirectAdminExtended\App\UI\\', '', trim($class, '\\'));
        }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\DirectAdminExtended\Core\Helper\di($parent, null, true);
                if ($new)
                {
