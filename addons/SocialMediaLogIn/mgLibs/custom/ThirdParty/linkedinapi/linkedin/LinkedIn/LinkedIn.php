...
//    const API_BASE = 'https://api.linkedin.com/v1';
    ...
    /* API v2*/
    const OAUTH_BASE_V2 = 'https://www.linkedin.com/oauth/v2';
    ...
    public function getAccessToken($authorization_code = null)
    {
        ...
        $data = $this->_makeRequest(self::OAUTH_BASE_V2 . '/accessToken?'.$tmp_params, $params, self::HTTP_METHOD_POST, array('x-li-format: json'));
    ...
    {
        $delimiter = strpos($endpoint,'?') ? '&' : '?';

        $endpoint = self::API_BASE . '/' . trim($endpoint, '/\\') . $delimiter .'oauth2_access_token=' . $this->getAccessToken();
