...
use \ModulesGarden\OvhVpsAndDedicated\Core\UI\Helpers\TemplateConstants;
...
trait Template
{
    ...
    protected function loadTemplateDir()
    {
        ...
        $base   = $isCore = null;
        if (str_contains($class, 'ModulesGarden\OvhVpsAndDedicated\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\OvhVpsAndDedicated\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\OvhVpsAndDedicated\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\OvhVpsAndDedicated\App\UI\\', '', trim($class, '\\'));
        }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\OvhVpsAndDedicated\Core\Helper\di($parent, null, true);
                if ($new)
                {
