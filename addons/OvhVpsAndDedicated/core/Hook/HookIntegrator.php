...
use function ModulesGarden\OvhVpsAndDedicated\Core\Helper\isAdmin;
...
class HookIntegrator
{
    ...
    protected function addIntegration($className = null)
    {
        ...
        $integrationClassName = '\ModulesGarden\OvhVpsAndDedicated\App\Integrations\\' . ($this->isAdmin ? 'Admin' : 'Client') . '\\' . $className;
        if (!class_exists($integrationClassName) || !is_subclass_of($integrationClassName, \ModulesGarden\OvhVpsAndDedicated\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    public function validateIntegrationInstance($instance = null)
    {
        ...
        if (!is_subclass_of($instance, \ModulesGarden\OvhVpsAndDedicated\Core\Hook\AbstractHookIntegrationController::class))
        {
    ...
    public function loadIntegrationData()
    {
        ...
        foreach ($this->integrations as $integration)
        {
            ...
            /** @var
             * $integrationResult \ModulesGarden\OvhVpsAndDedicated\Core\UI\View
             */

            $integrationResult = call_user_func([di($callbackData[0]), $callbackData[1]]);

            if (!($integrationResult instanceof \ModulesGarden\OvhVpsAndDedicated\Core\UI\View))
            {
    ...
    public function isIntegrationApplicable($integration = null)
    {
        ...
        if ((!is_subclass_of($integrationCallback[0], \ModulesGarden\OvhVpsAndDedicated\Core\Http\AbstractController::class)
             && !is_subclass_of($integrationCallback[0], \ModulesGarden\OvhVpsAndDedicated\Core\Http\AbstractClientController::class))
