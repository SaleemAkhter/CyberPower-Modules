...
use ModulesGarden\WordpressManager\App\Models\ThemeBlocked as ThemeBlockedModel;
...
class ThemeBlocked extends Command
{
    ...
    {
        $io->title('Delete Blocked Themes: Running');
        ...
        $output->writeln("");
        $io->success([
            sprintf("Deleted Blocked Themes: %s Entries Processed.", $synchronizedTasks),
            "Delete Blocked Themes: Done"
        ]);
        if ($this->hosting)
        {
            $settings                        = $this->hosting->productSettings->getSettings();
            $settings['themeCrobJobLastRun'] = date("Y-m-d H:i:s");
    ...
    private function scanInteral()
    {
        ...
        if (!$this->hosting->productSettings->getThemeCrobJobLastRun())
        {
            return true;
        }
        $lastRun = new DateTime($this->hosting->productSettings->getThemeCrobJobLastRun());
    ...
    private function installationProcess(Installation $installation, $output)
    {
        ...
        foreach ($this->subModule()->getThemes($installation) as $record)
        {
            if (!ThemeBlockedModel::where("slug", $record['name'])->where('product_id', $this->getHosting()->packageid)->count())
            {
                continue;
            }
            $this->subModule()->getTheme($installation)->delete($record['name']);
            Helper\infoLog(sprintf("Blocked Theme '%s' has been automatically deleted, Installation ID #%s, Client ID #%s, Hosting ID #%s\r\n",
                $record['name'], $installation->id, $installation->user_id, $installation->hosting_id));
            $output->writeln(sprintf("Blocked Theme '%s' has been automatically deleted, Installation ID #%s, Client ID #%s, Hosting ID #%s", $record['name'], $installation->id, $installation->user_id, $installation->hosting_id));
        }
    ...
    private function loopHostings($product)
    {
        ...
        foreach ($query->get() as $hosting)
        {
            ...
            try
            {
                ...
                //is enabled
                if (!$this->getHosting()->productSettings->isThemeBlockedDelete())
                {
                    continue;
                }
                //last run
    ...
    private function loopInstallations($hosting)
    {
        ...
        foreach (Installation::where("hosting_id", $hosting->id)->get() as $installation)
        {
            ...
            {
                Helper\errorLog(sprintf("Delete Blocked Themes:  '%s', Installation ID #%s, Client ID #%s, Hosting ID #%s\r\n", $ex->getMessage(), $installation->id, $installation->user_id, $installation->hosting_id));
                $this->io->error($ex->getMessage());
            }
        }
