...
mgJsComponentHandler.addDefaultComponent('mg-service-form', {
    ...
    data: function () {
        return {
            remoteSearchFields: ['clients'],
            remoteSourceFields: ['services', 'pricingGroups', 'credentials', 'billingTypes'],
            customFields: [],
            options: {},
            selectedValues: {},
            selectInstances: {}
    ...
    methods: {
        ...
        selectChangedCallback: function(fieldName, value) {
            if (fieldName === 'clients'){
                this.loadRemoteSourceData('services');
            }
            else if (fieldName === 'services'){
                this.loadRemoteSourceData('pricingGroups');
            }
            else if (fieldName === 'pricingGroups'){
                this.loadRemoteSourceData('credentials');
                this.loadRemoteSourceData('billingTypes');
            }
            else if (fieldName === 'billingTypes'){
        ...
        selectizeRawSelects: function(){
            ...
            $.each(JSON.parse(JSON.stringify(this.customFields)), function(id, field) {
                if(field.type == 'select') {
                    customSelect = $("[name=" + field.name + "]");
                    while(customSelect.length > 1) {
                        customSelect[0].remove();
                    }
                    if(typeof customSelect[0].selectize != 'undefined') {
                        customSelect[0].selectize.destroy()
                    }
                    customSelect
                        .find('option')
                        .remove()
                    ...
                    $.each(field.data, function(id, data) {
                        customSelect
