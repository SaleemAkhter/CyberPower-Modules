...
// custom integration for service page

function servicePageIntegrate(target, content)
{
    $('#subscription').parents('tr').first().before('<tr><td class="fieldlabel" width="20%">Cloud Billing</td><td class="fieldarea" colspan="3"><div id="mg-cloud-billing-integration"></td></tr>');

    $('#mg-cloud-billing-integration').html($('.mg-integration-container').html());
    var modalContainer = $('.vue-app-main-modal-container').first()[0].outerHTML;
    $('.vue-app-main-modal-container').remove();

    $('.mg-integration-container').after('<div id="layers">' + modalContainer + '</div>');

    $('.mg-integration-container').remove();
}
    ...
    methods: {
        ...
        loadCustomFields: function(){
            ...
            var response = this.loadAjaxData({index: 'customFields', formData: this.getFormData()}).done(function(data){
                if (typeof data.data.rawData.fields !== 'undefined'){
