...
mgJsComponentHandler.addDefaultComponent('category-menu', {
    ...
    created: function () {
        var self = this;
        self.gid = mgUrlParser.getUrlParam('gid', null);
    ...
    methods: {
        ...
        loadCategories : function(callback){
            ...
            resp.done(function(data){
                ...
                if (typeof data.data.rawData != 'undefined' && data.data.rawData.length > 0) {
                    if(callback){
                        if (self.activeGroupId || self.gid !== null) {
                            for (var key in data.data.rawData) {
                                if (!data.data.rawData.hasOwnProperty(key)) {
                                    continue;
                                } else {
                                    if (self.gid !== null && data.data.rawData[key].id == self.gid) {
                                        self.gid = null;
                    ...
                    self.hideNoGroupContainer();
                } else {
                    self.showNoGroups = true;
                    self.showNoGroupContainer();
                }
            });
        },
        ...
        isRowActive: function(dataRow, index) {
            var self = this;
            if (self.gid === null && index === 0){
                return true;
            } else if (self.gid !== null && dataRow.id == self.gid) {
                return true;
            } else {
                return false;
            }
        },
        showNoGroupContainer: function(){
            $('#' + this.component_id).parents('.lu-block').first().find('#mgShowNoGroups').removeAttr('hidden');
        },
        hideNoGroupContainer: function(){
            $('#' + this.component_id).parents('.lu-block').first().find('#mgShowNoGroups').attr('hidden', 'hidden');
        },
        // },
        // makeCustomAction: function (functionName, params, event, namespace, index) {
        //     mgPageControler.vueLoader.makeCustomAction(functionName, params, event, namespace, index);
