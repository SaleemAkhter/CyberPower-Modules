...
use ModulesGarden\DirectAdminExtended\Core\App\Requirements\Instances\Files as FilesInstance;
...
class Files extends Handler implements HandlerInterface
{
    ...
    public function getFullPath($recordPath = null)
    {
        ...
        {
            return str_replace(FilesInstance::WHMCS_PATH, \ModulesGarden\DirectAdminExtended\Core\ModuleConstants::getFullPathWhmcs(),
                    str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }

        if (stripos($recordPath, FilesInstance::MODULE_PATH) === 0)
        {
            return str_replace(FilesInstance::MODULE_PATH, \ModulesGarden\DirectAdminExtended\Core\ModuleConstants::getModuleRootDir(),
                    str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }
    ...
    {
        $fileValidator = new \ModulesGarden\DirectAdminExtended\Core\FileReader\PathValidator();
