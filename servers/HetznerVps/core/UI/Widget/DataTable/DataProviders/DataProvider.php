...
namespace ModulesGarden\Servers\HetznerVps\Core\UI\Widget\DataTable\DataProviders;
...
{
    const FILTR_BY_DATE           = '\ModulesGarden\Servers\HetznerVps\Core\UI\Widget\DataTable\Filters\Date';
    const FILTR_BY_RAGE           = '\ModulesGarden\Servers\HetznerVps\Core\UI\Widget\DataTable\Filters\Rage';
    const FILTR_BY_RAGE_DATE      = '\ModulesGarden\Servers\HetznerVps\Core\UI\Widget\DataTable\Filters\RageDate';
    const FILTR_BY_SELECT         = '\ModulesGarden\Servers\HetznerVps\Core\UI\Widget\DataTable\Filters\Select';
    const FILTR_BY_TEXT           = '\ModulesGarden\Servers\HetznerVps\Core\UI\Widget\DataTable\Filters\Text';
    const FILTR_BY_YESNO          = '\ModulesGarden\Servers\HetznerVps\Core\UI\Widget\DataTable\Filters\YesNo';
    const SORT_ASC                = 'ASC';
    ...
    protected function useSearch($data = [])
    {
        ...
        {
            $searchable = [];
            foreach ($this->availableCols as $column)
            {
    ...
    public function getData(array $availableCols = [])
    {
    ...
    {
        if ($this->orderColumn && $this->orderDir && $this->availableCols[$this->orderColumn])
        {
            $column = $this->availableCols[$this->orderColumn];
    ...
    protected function sortNow($fieldName, $type, $asc)
    {
        if ($type == 'string')
        {
            usort($this->records, function (array $a, array $b) use ($fieldName, $asc)
            {
                if ($asc)
                {
                    return strnatcmp(strtolower($a[$fieldName]), strtolower($b[$fieldName]));
                }

                return strnatcmp(strtolower($b[$fieldName]), strtolower($a[$fieldName]));
            });
        }
        elseif ($type == 'int')
        {
