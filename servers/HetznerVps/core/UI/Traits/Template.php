...
use \ModulesGarden\Servers\HetznerVps\Core\UI\Helpers\TemplateConstants;
...
trait Template
{
    ...
    protected function loadTemplateDir()
    {
        ...
        $base   = $isCore = null;
        if (str_contains($class, 'ModulesGarden\Servers\HetznerVps\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\Servers\HetznerVps\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\Servers\HetznerVps\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\Servers\HetznerVps\App\UI\\', '', trim($class, '\\'));
        }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\Servers\HetznerVps\Core\Helper\di($parent, null, true);
                if ($new)
                {
