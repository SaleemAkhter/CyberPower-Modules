...
use ModulesGarden\Servers\HetznerVps\App\Libs\HetznerVps\Helpers\CreateVM;
...
class CreateAccount extends AddonController
{
    ...
    public function execute($params = null)
    {
        ...
        try {
            $createManager = new CreateVM($this->params);
            $serverID = $createManager->create();

            $floatingIps = $this->api->getClient()->getIps();
            $toshow = '';
            for ($i = 0; $i < $floatingIps; $i++) {
                $toshow .= 'cr- ';
                RegisterTask::createFloatingIP($serverID, $this->params);
            }
            CustomFields::set($this->params['serviceid'], 'serverID', $serverID);
            $this->backupsPermission($serverID);
        } catch (Exception $ex) {
