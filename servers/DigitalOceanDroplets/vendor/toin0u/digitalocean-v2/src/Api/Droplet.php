...
class Droplet extends AbstractApi
{
    ...
    public function listOfTags(){
        $url = sprintf('%s/tags', $this->endpoint);
        return json_decode($this->adapter->get($url));
    }
    
    public function createTag($tag){
        $url = sprintf('%s/tags', $this->endpoint);
        
        $this->adapter->post($url, ['name' => $tag]);
    }
    
    public function tagAResource($tag, $resources){
        $listOfTagname = [];
        
        foreach($this->listOfTags()->tags as $tagObj){
            array_push($listOfTagname, $tagObj->name);
        }
        
        if(!in_array($tag, $listOfTagname)){
            $this->createTag($tag);
        }
                   
        $url = sprintf('%s/tags/%s/resources', $this->endpoint, $tag);

        $this->adapter->post($url, $resources);
    }
    
    public function untagAResource($tag, $resources){
        $url = sprintf('%s/tags/%s/resources', $this->endpoint, $tag);      

        $this->adapter->delete($url, $resources);
    }
    
    public function getDropletAction($dropletID, $actionID)
    {
        $result = $this->adapter->get(sprintf('%s/droplets/%s/actions/%s', $this->endpoint, $dropletID, $actionID));
