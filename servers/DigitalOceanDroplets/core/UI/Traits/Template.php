...
use \ModulesGarden\Servers\DigitalOceanDroplets\Core\UI\Helpers\TemplateConstants;
...
trait Template
{
    ...
    protected function loadTemplateDir()
    {
        ...
        $base   = $isCore = null;
        if (str_contains($class, 'ModulesGarden\Servers\DigitalOceanDroplets\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\Servers\DigitalOceanDroplets\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\Servers\DigitalOceanDroplets\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\Servers\DigitalOceanDroplets\App\UI\\', '', trim($class, '\\'));
        }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\Servers\DigitalOceanDroplets\Core\Helper\di($parent, null, true);
                if ($new)
                {
