...
namespace ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Http\View;
        ...
        foreach ($this->menuContect as $catName => $category)
        {
            if (isset($category['submenu']))
            {
                foreach ($category['submenu'] as $subName => &$subPage)
                {
                    ...
                    {
                        $subPage['url'] = isset($subPage['externalUrl']) ?: BuildUrl::getUrl($catName, $subName);
                    }
                }
            }

            if (!is_string($category)) {
                $category['url'] = isset($category['externalUrl']) ?: BuildUrl::getUrl($catName);
            }
