...
namespace ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Requirements\Handlers;

use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Requirements\Handler;
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Requirements\HandlerInterface;
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Requirements\Instances\Files as FilesInstance;
...
class Files extends Handler implements HandlerInterface
{
    ...
    public function getFullPath($recordPath = null)
    {
        ...
        {
            return str_replace(FilesInstance::WHMCS_PATH, \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\ModuleConstants::getFullPathWhmcs(),
                    str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }

        if (stripos($recordPath, FilesInstance::MODULE_PATH) === 0)
        {
            return str_replace(FilesInstance::MODULE_PATH, \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\ModuleConstants::getModuleRootDir(),
                    str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }
    ...
    {
        $fileValidator = new \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\FileReader\PathValidator();
