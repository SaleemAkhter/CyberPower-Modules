...
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Controllers\ResponseResolver;
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Installer\ModuleInstaller;
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Helper\WhmcsVersionComparator;
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Http\JsonResponse;
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Models\Whmcs\Product;
use ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Models\Whmcs\Server;
 ...
 */
class ConfigOptions extends \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Controllers\Instances\AddonController
{
    use AppParams;
    ...
    public function execute($params = null)
    {
        ...
        if (($this->getRequestValue('action') === 'module-settings' || ($this->getRequestValue('loadData') && $this->getRequestValue('ajax') == '1')))
        {
            try
            {
                ...
                $requirementsHandler = new \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\App\Requirements\Checker();
            ...
            try
            {
                $this->setAppParam('IntegrationControlerName', \ModulesGarden\Servers\GoogleCloudVirtualMachines\App\Http\Actions\ConfigOptions::class);
                $this->setAppParam('IntegrationControlerMethod', 'runExecuteProcess');

                $configOptionsController = new \ModulesGarden\Servers\GoogleCloudVirtualMachines\App\Http\Actions\ConfigOptions();
        ...
        else if ($this->getRequestValue('action') === 'save')
        {
            ...
            return [\ModulesGarden\Servers\GoogleCloudVirtualMachines\App\Http\Actions\ConfigOptions::class, 'runExecuteProcess'];
        }
    ...
    public function isCorrectServerType()
    {
        try
        {
            if (class_exists('\ModulesGarden\Servers\GoogleCloudVirtualMachines\App\Http\Actions\MetaData'))
            {
                $metaDataController = new \ModulesGarden\Servers\GoogleCloudVirtualMachines\App\Http\Actions\MetaData();
    ...
    public function addRequiredCustomFields()
    {
        ...
        $product = new \ModulesGarden\Servers\GoogleCloudVirtualMachines\Packages\WhmcsService\Product($pid);
