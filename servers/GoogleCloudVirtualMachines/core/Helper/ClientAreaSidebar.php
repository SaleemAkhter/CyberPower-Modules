...
class ClientAreaSidebar
{
    ...
    use \ModulesGarden\Servers\GoogleCloudVirtualMachines\App\Traits\HostingComponent;
    ...
    {       
        ...
        if (!$this->hosting || !$this->request->getSession('uid') || $this->hosting->domainstatus !== 'Active' || $this->hosting->product->servertype !== 'GoogleCloudVirtualMachines')
        {
            return;
        }
...
//            'uri' => BuildUrl::getUrl('OneClickLogin', 'redirect',array_merge($params,['where' => $childName])),
    ...
    public function addSubmenuItem(&$primaryMenuItem, $name, $menuItem)
    {
        ...
        $primaryMenuItem->addChild($name, [
            ...
            'uri'     => '#',
            'icon'    => is_array($menuItem) ? $menuItem['icon'] : '',
    ...
    public function isProperProductType(){
        ...
        return $this->hosting->product->servertype === "GoogleCloudVirtualMachines";
    }
