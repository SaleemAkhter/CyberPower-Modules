...
use function ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Helper\isAdmin;
...
class HookIntegrator
{
    ...
    protected function addIntegration($className = null)
    {
        ...
        $integrationClassName = '\ModulesGarden\Servers\GoogleCloudVirtualMachines\App\Integrations\\' . ($this->isAdmin ? 'Admin' : 'Client') . '\\' . $className;
        if (!class_exists($integrationClassName) || !is_subclass_of($integrationClassName, \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    {
        if (!is_subclass_of($instance, \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    public function loadIntegrationData()
    {
        foreach ($this->integrations as $integration)
        {
            ...
            /** @var
             * $integrationResult \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\UI\View
             */
            $integrationResult = call_user_func([di($callbackData[0]), $callbackData[1]]);
            if (!($integrationResult instanceof \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\UI\View))
            {
    ...
    public function isIntegrationApplicable($integration = null)
    {
        ...
        if ((!is_subclass_of($integrationCallback[0], \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Http\AbstractController::class)
            && !is_subclass_of($integrationCallback[0], \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Http\AbstractClientController::class))
