...
use \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\UI\Helpers\TemplateConstants;
...
trait Template
{
    ...
    protected function loadTemplateDir()
    {
        ...
        $base   = $isCore = null;
        if (str_contains($class, 'ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\Servers\GoogleCloudVirtualMachines\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\Servers\GoogleCloudVirtualMachines\App\UI\\', '', trim($class, '\\'));
        }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\Servers\GoogleCloudVirtualMachines\Core\Helper\di($parent, null, true);
                if ($new)
                {
