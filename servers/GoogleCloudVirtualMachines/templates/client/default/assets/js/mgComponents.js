...
function tldCategoriesSortableController() 
{
    ...
    if (! $('#groupList.vSortable').hasClass('ui-sortable'))
    {
        $("#groupList.vSortable").sortable(
        {
            ...
            stop: function(event, ui)
            {
                ...
                mgPageControler.vueLoader.updateSorting(order, 'addCategoryForm', 'ModulesGarden_GoogleCloudVirtualMachines_App_UI_Widget_DoeTldConfigComponents_CategoryForms_AddCategoryForm');
    ...
    $('#itemContentContainer tbody.vSortable').sortable(
    {
        stop: function(event, ui)
        {
            ...
            mgPageControler.vueLoader.updateSorting(order, 'assignTldForm', 'ModulesGarden_GoogleCloudVirtualMachines_App_UI_Widget_DoeTldConfigComponents_CategoryForms_AssignTldForm');
        },
...
function redirectRdp(event, params, targetBlank) {
    mgPageControler.vueLoader.redirect(event, params, targetBlank);
}

function replaceTextInputWithPassword(data) {
    $('#resetPasswordForm .hidden').removeClass('hidden');
    $('#resetPasswordForm input[type="text"]').val(data.rawData).css({'color' : "#000", "background-color": "#fff"});
    $('#confirmationModal .lu-col-md-12:first-child').contents()[0].nodeValue = '';
    // $('.lu-modal__actions button').first().text("Copy").removeClass("submitForm").removeClass("mg-submit-form").addClass("dpcopy");
    $('.lu-modal__actions').addClass("hide");
    $('#resetPasswordForm input[type="text"]').removeAttr("disabled");
    copyToClipboard('#resetPasswordForm input[type="text"]');  
}


function copyToClipboard(element) {
    // console.log($(element).val());
    var $temp = $('#resetPasswordForm input[type="text"]');
    $temp.focus();
    $temp.select();
