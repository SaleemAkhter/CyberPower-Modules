...
function kametera_gen_pass($le = 10)
{
...
function kametera_CreateAccount(array $params)
{
    ...
    $params['password']=kametera_gen_pass(rand(18,32));

    try 
    {
        /*
         * To manage product welcome email
         */
        kametera_createServerTable();
        
        //kametera_saveServerDetail($params["serviceid"]);
...
function kametera_createServerTable()
{
    try 
    {
        if(!Capsule::schema()->hasTable('kametera_welcome_email_stats'))
        {
            Capsule::schema()->create('kametera_welcome_email_stats',function ($table) 
            {
                $table->increments('id');
    ...
    {
        logActivity("Unable to create kametera_welcome_email_stats table: {$e->getMessage()}");
    }
}

function kametera_saveServerDetail($serviceId)
{
    try
    {
        /*
         * check if service id is already exist
         */
        $id = Capsule::table('kametera_welcome_email_stats')->where('serviceid',$serviceId)->value('id');
        ...
        if(!empty($id))
        {
             ...
             */
            Capsule::table("kametera_welcome_email_stats")->where('id',$id)->update(['emailsent' => 0]);
        }
        else
        {
            /*
            ...
            Capsule::table("kametera_welcome_email_stats")->insert(['serviceid' => $serviceId,'emailsent' => 0]);
        }
    ...
    {
        logActivity("failed to insert kametera_welcome_email_stats. Error: {$ex->getMessage()}");
    }
}
