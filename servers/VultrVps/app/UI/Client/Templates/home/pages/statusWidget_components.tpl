...
>
    <div class="lu-col-md-12 mg-status-widget">
        ...
        {if !$customTplVars['isAdmin']}
            <div class="lu-h4 lu-m-b-3x lu-m-t-2x">{$MGLANG->translate('Service Managament')}</div>
            <div class="lu-tiles lu-row lu-row--eq-height">
                {foreach from=$rawObject->getManagamentButtons() key=buttonKey item=buttonValue}
                    {$buttonValue->getHtml()}
                {/foreach}
            </div>
        {/if}
            ...
            <div class="lu-col-md-12" v-for="widget in data.instanceDetails">
                <div class="lu-widget">
                    ...
                    <div class="lu-widget__body">
                        <table v-if="widget.details.length > 0"
                               class="lu-table lu-table--mob-collapsible dataTable no-footer dtr-column" width="100%"
                               role="grid">
                            <tbody>
                    ...
                    <div class="lu-preloader-container lu-preloader-container--full-screen lu-preloader-container--overlay widget-only"
                         v-show="loadingState">
