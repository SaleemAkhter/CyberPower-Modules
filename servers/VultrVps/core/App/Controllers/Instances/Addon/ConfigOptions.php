...
use ModulesGarden\Servers\VultrVps\Core\App\Installer\ModuleInstaller;
use ModulesGarden\Servers\VultrVps\Core\Http\JsonResponse;
use ModulesGarden\Servers\VultrVps\Core\Models\Whmcs\Product;
use ModulesGarden\Servers\VultrVps\Core\Models\Whmcs\Server;
 ...
 */
class ConfigOptions extends \ModulesGarden\Servers\VultrVps\Core\App\Controllers\Instances\AddonController
{
    use AppParams;
    ...
    public function execute($params = null)
    {
        ...
        if (($this->getRequestValue('action') === 'module-settings' || ($this->getRequestValue('loadData') && $this->getRequestValue('ajax') == '1')))
        {
            try
            {
                ...
                $requirementsHandler = new \ModulesGarden\Servers\VultrVps\Core\App\Requirements\Checker();
            ...
            try
            {
                $this->setAppParam('IntegrationControlerName', \ModulesGarden\Servers\VultrVps\App\Http\Actions\ConfigOptions::class);
                $this->setAppParam('IntegrationControlerMethod', 'runExecuteProcess');

                $configOptionsController = new \ModulesGarden\Servers\VultrVps\App\Http\Actions\ConfigOptions();
        ...
        else if ($this->getRequestValue('action') === 'save')
        {
            ...
            return [\ModulesGarden\Servers\VultrVps\App\Http\Actions\ConfigOptions::class, 'runExecuteProcess'];
        }
    ...
    public function isCorrectServerType()
    {
        try
        {
            if (class_exists('\ModulesGarden\Servers\VultrVps\App\Http\Actions\MetaData'))
            {
                $metaDataController = new \ModulesGarden\Servers\VultrVps\App\Http\Actions\MetaData();
    ...
    public function buildErrorMessage($message)
    {
        $data = [
            'content' =>
                '<div style="width=100%; margin-bottom: 4px;" class="alert alert-danger">' . $message . '</div>',
            'mode' => 'advanced'
        ];
    ...
    public function addRequiredCustomFields()
    {
        ...
        $product = new \ModulesGarden\Servers\VultrVps\Packages\WhmcsService\Product($pid);
