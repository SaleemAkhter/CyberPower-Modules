...
use ModulesGarden\Servers\VultrVps\Core\App\Requirements\Instances\Files as FilesInstance;
...
class Files extends Handler implements HandlerInterface
{
    ...
    public function getFullPath($recordPath = null)
    {
        ...
        {
            return str_replace(FilesInstance::WHMCS_PATH, \ModulesGarden\Servers\VultrVps\Core\ModuleConstants::getFullPathWhmcs(),
                    str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }

        if (stripos($recordPath, FilesInstance::MODULE_PATH) === 0)
        {
            return str_replace(FilesInstance::MODULE_PATH, \ModulesGarden\Servers\VultrVps\Core\ModuleConstants::getModuleRootDir(),
                    str_replace('/', DIRECTORY_SEPARATOR, $recordPath));
        }
    ...
    {
        $fileValidator = new \ModulesGarden\Servers\VultrVps\Core\FileReader\PathValidator();
