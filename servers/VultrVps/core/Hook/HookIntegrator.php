...
class HookIntegrator
{
    ...
    protected function addIntegration($className = null)
    {
        ...
        $integrationClassName = '\ModulesGarden\Servers\VultrVps\App\Integrations\\' . ($this->isAdmin ? 'Admin' : 'Client') . '\\' . $className;
        if (!class_exists($integrationClassName) || !is_subclass_of($integrationClassName, \ModulesGarden\Servers\VultrVps\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    {
        if (!is_subclass_of($instance, \ModulesGarden\Servers\VultrVps\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    public function loadIntegrationData()
    {
        foreach ($this->integrations as $integration)
        {
            ...
            /** @var
             * $integrationResult \ModulesGarden\Servers\VultrVps\Core\UI\View
             */
            $integrationResult = call_user_func([di($callbackData[0]), $callbackData[1]]);
            if (!($integrationResult instanceof \ModulesGarden\Servers\VultrVps\Core\UI\View))
            {
    ...
    public function isIntegrationApplicable($integration = null)
    {
        ...
        if ((!is_subclass_of($integrationCallback[0], \ModulesGarden\Servers\VultrVps\Core\Http\AbstractController::class)
            && !is_subclass_of($integrationCallback[0], \ModulesGarden\Servers\VultrVps\Core\Http\AbstractClientController::class))
