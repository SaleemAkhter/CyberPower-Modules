...
use ModulesGarden\Servers\DirectAdminExtended\Core\Models\Whmcs as wModels;;
use function ModulesGarden\Servers\DirectAdminExtended\Core\Helper\sl;


use ModulesGarden\Servers\DirectAdminExtended\Core\Configuration\Addon;
...
{
    /**
     * @var \ModulesGarden\Servers\DirectAdminExtended\Core\Helper\Lang
     */
    ...
    private function __construct($dir = null, $lang = null)
    {
        ...
        if ($this->isDebug)
        {
            //$this->isDebug = (bool)(int)sl('configurationAddon')->getConfig('debug', 0);
        }
     ...
     * @param type $dir
     * @param type $lang
     * @return \ModulesGarden\Servers\DirectAdminExtended\Core\Helper\Lang
     */
    ...
    public function inlcludeWordpres()
    {
        if(class_exists('\ModulesGarden\WordpressManager\Core\ModuleConstants'))
        {
            if (!$this->loadWordpressFile($this->currentLang))
            {
                $this->loadWordpressFile('english');
            }
        }
    }
    
    public function loadWordpressFile($fileName)
    {
        $wpLangPath = \ModulesGarden\WordpressManager\Core\ModuleConstants::getLangsDir() . DIRECTORY_SEPARATOR;
        if (file_exists($wpLangPath . $fileName . '.php'))
        {
            include $wpLangPath . $fileName . '.php';

            foreach ($_LANG['addonCA']['home'] as $key => $value)
            {
                if (!$this->langs[$key])
                {
