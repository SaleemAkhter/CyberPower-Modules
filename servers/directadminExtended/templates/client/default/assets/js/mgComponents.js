...
function initContainerTooltips(elId) {
...
function changeUnlimitedFields(field){
    if(jQuery("input[name='"+field+"Unlimited']").is(":checked")){
        jQuery("input[name='"+field+"Unlimited']").closest(".lu-form-group").find("input[type='text']").prop("disabled","disabled");
    }else{
        jQuery("input[name='"+field+"Unlimited']").closest(".lu-form-group").find("input[type='text']").prop("disabled","");
    }
}
function changeUnlimitedFieldsOld(field){
    if(jQuery("input[name='"+field+"Unlimited']").is(":checked")){
        jQuery("input[name='"+field+"Unlimited']").closest(".lu-form-check").next(".lu-form-group").hide();
    }else{
        jQuery("input[name='"+field+"Unlimited']").closest(".lu-form-check").next(".lu-form-group").show();
    }
}
...
function initModalSwitchEvents(){
    ...
    initModalRadioEvents();
}
function initModalRadioEvents(){
    $('#mgModalContainer input[type="radio"]').each(function(){
        if ($(this).attr('bi-event-change')) {
            var biEventAction = $(this).attr('bi-event-change');
            $(this).change(function() {
                console.log(mgPageControler);
                    if (biEventAction && typeof mgPageControler[biEventAction] === "function") {
                        setTimeout(function(){
                            mgPageControler[biEventAction]();
                        }, 500);
...
function mgFormControler(targetFormId) {
    ...
    this.getFieldsData = function() {
        ...
        for (var key in this.data) {
            ...
            // console.log(formatedKey,typeof KeyValue);
            if(typeof KeyValue === 'object') {
                ...
                // console.log(formatedKey,KeyValue,formatedData);
    ...
    this.updateFieldsValidationMessages = function(errorsList) {
        jQuery('#'+this.formId).find('input,select,textarea').each(function () {
            if (!jQuery(this).is(':disabled')) {
                ...
                if(name !== undefined && errorsList[name] === undefined) {
                    ...
                    var messagePlaceholder = jQuery(this).parents('.lu-form-group').first().children('.lu-form-feedback').first();
        ...
        jQuery('#'+this.formId).find('input,select,textarea').each(function () {
            if (!jQuery(this).is(':disabled')) {
                var name = jQuery(this).attr('name');
                if(name !== undefined && errorsList !== undefined && Object.keys(errorsList).length > 0 && errorsList[name] !== undefined)
                {
                    if(!jQuery(this).parents('.lu-form-group').first().hasClass('lu-is-error')) {
...
function tldCategoriesSortableController()
{
    ...
    if (! $('#groupList.vSortable').hasClass('ui-sortable'))
    {
        $("#groupList.vSortable").sortable(
        {
            ...
            stop: function(event, ui)
            {
                ...
                mgPageControler.vueLoader.updateSorting(order, 'addCategoryForm', 'ModulesGarden_DirectAdminExtended_App_UI_Widget_DoeTldConfigComponents_CategoryForms_AddCategoryForm');
    ...
    $('#itemContentContainer tbody.vSortable').sortable(
    {
        stop: function(event, ui)
        {
            ...
            mgPageControler.vueLoader.updateSorting(order, 'assignTldForm', 'ModulesGarden_DirectAdminExtended_App_UI_Widget_DoeTldConfigComponents_CategoryForms_AssignTldForm');
        },
...
function hideFileManagerOptions(data){
    setTimeout(function(){
        $("#fileManagerPage .dataTables_wrapper tbody tr").each(function(){
            if($(this).find('.lu-zmdi-folder').length == 0 || !data.rawData.additionalData.currentDirPath.includes('public_html'))
            {
                $(this).find('.lu-zmdi-lock').closest('.lu-dropdown__item').hide();
            }
            else
            {
                $(this).find('.lu-zmdi-lock').closest('.lu-dropdown__item').show();
            }
        });
    }, 500);
}

function redirectAfterFormSubmit(data){
    setTimeout(function(){
        window.location.href=(data.rawData.url);
    }, 500);
}
jQuery(document).ready(function(){
    jQuery("select[name='subjectcontains']").on("change",function(){
        var selected=jQuery(this).next(".selectize-control").find(".selectize-input").find("[data-value='"+jQuery(this).val()+"']").text().trim();
         console.log(selected);
         jQuery("#subject").val(selected);
    });
    jQuery("select[name='when']").on("change",function(){
        var selected=jQuery(this).next(".selectize-control").find(".selectize-input").find("[data-value='"+jQuery(this).val()+"']").text().trim();
        console.log(selected);
        if(selected!="-- any time --"){
            jQuery("#delete_messages_days").prop("disabled","");
        }else{
            jQuery("#delete_messages_days").val("").prop("disabled","disabled");
        }
    });
    jQuery(document).on('mouseenter', '.selectize-control', function() {
      // alert($(this).text())
      // console.log(jQuery(this).prev('select'));
      // jQuery(this).prev('select').selectize().open();
    });
    jQuery(document).on('mouseleave', '.selectize-control', function() {
      // jQuery(this).prev('select').selectize().close();
    });
