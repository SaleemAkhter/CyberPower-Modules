mgJsComponentHandler.addDefaultComponent('mg-datatable', {
    ...
    methods: {
        ...
        updateProjects: function(){
            ...
            resp.done(function(data){
                if (data.data.status === 'error') {
                    ...
                    setTimeout(() => {
                      jQuery("[data-hidebuttoncolumn]").each(function(){
                        index = self.dataRows.findIndex(x => x.id ===jQuery(this).closest('tr').attr('actionid'));
                        var col=jQuery(this).data('hidebuttoncolumn');
                        var v=jQuery(this).data('hidebuttonvalue');
                        if(self.dataRows[index][col]==v){
                            jQuery(this).closest('.lu-dropdown__item').hide();
                        }else{
                            jQuery(this).closest('.lu-dropdown__item').show();
                        }
                      });
                    }, 600);
                }
        ...
        redirectToEditUrl:function(targetUrl, event){
            ...
            window.location.href = targetUrl+"&aaaa=1&actionElementId="+actionid;
