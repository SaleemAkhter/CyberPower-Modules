...
use ModulesGarden\Servers\DirectAdminExtended\Core\Http\View\Smarty;
...
use ModulesGarden\Servers\DirectAdminExtended\Core\Helper\WhmcsVersionComparator;
...
class Response extends SymfonyRespose
{
    ...
    /**
     * @param $responseResolver \ModulesGarden\Servers\DirectAdminExtended\Core\App\Controllers\ResponseResolver
     */
    public function getHtmlResponse($responseResolver)
    {
        ...
        $vars = [
            ...
            'resellerImpersonated'=> (Helper\isResellerLevel() && !defined("RESELLER_PAGE")),
            'unreadmessagecount'=> (isset($_SESSION['unreadmessagecount']))?$_SESSION['unreadmessagecount']:"",
    ...
    public function returnClientAddon($vars, $path, $fileName)
    {
        return [
            'vars' => $vars,
            'templatefile' => str_replace(\ModulesGarden\Servers\DirectAdminExtended\Core\ModuleConstants::getTemplateDir() . DIRECTORY_SEPARATOR,
    ...
    public function returnClientProvisioning($vars, $path, $fileName)
    {
        ...
        return [
            $templateVarName => str_replace(\ModulesGarden\Servers\DirectAdminExtended\Core\ModuleConstants::getTemplateDir() . DIRECTORY_SEPARATOR,
                '', $path . DIRECTORY_SEPARATOR . $fileName),
    ...
    {
        $requirementsHandler = new \ModulesGarden\Servers\DirectAdminExtended\Core\App\Requirements\Checker();
