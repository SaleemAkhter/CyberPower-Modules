...
class ClientAreaSidebar
{
    ...
    public function addChildren(&$menuItem, $children)
    {
        ...
        foreach ($children as $childName => $child)
        {
            ...
            if(!Helper\isResellerLevel() && $this->product->type === 'reselleraccount' && $this->isFeatureEnabled(array_search(ucfirst($childName), ResellersFeaturesWithControllers::get()), $this->productId) === false)
            {
                continue;
            }
            if ($this->product->type !== 'reselleraccount' && $this->isFeatureEnabled(array_search(ucfirst($childName), FeaturesWithControllers::get()), $this->productId) === false)
            {
                continue;
            }

            $setting = array_search(ucfirst($childName), FeaturesWithControllers::get());
            $resellersetting = array_search(ucfirst($childName), ResellersFeaturesWithControllers::get());
            if (!$this->isFeatureEnabled($setting, $this->productId) && (!$this->isFeatureEnabled($resellersetting, $this->productId))) {
                continue;
            }
            if($resellersetting && Helper\isResellerLevel()){
                continue;
            }
