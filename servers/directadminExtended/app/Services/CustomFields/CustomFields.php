...
namespace ModulesGarden\Servers\DirectAdminExtended\App\Services\CustomFields;

use ModulesGarden\Servers\DirectAdminExtended\Core\Models\Whmcs\CustomField;
use ModulesGarden\Servers\DirectAdminExtended\Core\Models\Whmcs\CustomFieldValue;
use ModulesGarden\Servers\DirectAdminExtended\Core\Models\Whmcs\Hosting;
...
class CustomFields
{
    ...
    public static function create($productID, $fieldName, $fieldType = 'text', $adminOnly = "on", $showOnOrder = "on", $requiredField = "on")
    {
        $fieldName = explode('|', $fieldName);
        $field     = self::getField($fieldName[0], $productID);
        if (is_null($field))
        {
            self::createField('product', $productID, implode('|', $fieldName), $fieldType, $adminOnly, $showOnOrder, $requiredField);
        }
    }
    ...
    private static function createField($type, $relID, $fieldName, $fieldType = "text", $adminOnly = "on", $showOnOrder = "on", $requiredField = "on")
    {
        ...
        $customField->showorder = $showOnOrder;
        $customField->required  = $requiredField;
        $customField->save();
        return $customField;
    }
