...
use ModulesGarden\Servers\DirectAdminExtended\App\UI\Client\Admin\UserManager\Providers\Users;
...
class AddNewUser extends BaseStandaloneForm implements ClientArea
{
    ...
    public function initContent()
    {
        ...
        if(empty($data['availablepackages'])){
            ...
            $message='Sorry, it seems that there are no user packages created yet. Would you like to create one?';
            $this->addField((new HtmlField('aa'))->setRawHtml('<p>'.$message.'</p>'));
            $submitButton = new ButtonSubmitForm();
            $submitButton->setFormId($this->id)->setTitle('Create User Package');
