...
namespace ModulesGarden\Servers\DirectAdminExtended\App\UI\Client\Admin\ListReseller\Pages;
...
use ModulesGarden\Servers\DirectAdminExtended\App\UI\Client\Admin\ListReseller\Buttons;
...
class ResellerTable extends DataTableApi implements ClientArea
{
    use WhmcsParams;
    ...
    protected function loadHtml()
    {
        ...
        ->addColumn((new Column('nusers'))->setTitle('# of Users')->setSearchable(true)->setOrderable(true))
        ->addColumn((new Column('vdomains'))->setTitle('# of Domains')->setSearchable(true)->setOrderable(true))
        ->addColumn((new Column('suspended'))->setTitle('Suspended')->setSearchable(false)->setOrderable(false));
    }
...
// date_created
    ...
    protected function loadData()
    {
        ...
        $result     = $this->adminApi->reseller->listResellerUsers();
        $users=[];
        foreach($result as $key=>$user){
            $users[$key]['id']=$users[$key]['username'] =$users[$key]['user'] = $user->username;
            $users[$key]['bandwidth']=$user->bandwidth;
            $users[$key]['disk']=$user->quota;
            $users[$key]['vdomains']=$user->vdomains;
            $users[$key]['nusers']=$user->nusers;
            $suspended=explode(":",$user->suspended->value);
            $users[$key]['suspended']=$suspended[0];
        }
