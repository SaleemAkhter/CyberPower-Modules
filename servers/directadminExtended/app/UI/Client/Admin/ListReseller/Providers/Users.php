...
class Users extends ProviderApi
{
    ...
    public function read()
    {
        ...
        $result     = $this->adminApi->adminPackage->list();
        $packageopts=[];
    ...
    public function create()
    {
        ...
        $response=$this->resellerApi->reseller->createUser(new Models\Command\User($data));

        return (new ResponseTemplates\RawDataJsonResponse([
            'url' => $this->getUsersIndexURL(),
        ]))->setMessageAndTranslate('userHasBeenCreated')->setCallBackFunction('redirectAfterFormSubmit');
    }
    ...
    public function delete()
    {
        ...
        $data = [
         ...
         $this->loadAdminApi();
         $response=$this->adminApi->reseller->deleteReseller(new Models\Command\User($data));
         if(isset($response->result)){
            $message=nl2br($response->result);
         }else{
            $message='reselleruserHasBeenDeleted';
         }
