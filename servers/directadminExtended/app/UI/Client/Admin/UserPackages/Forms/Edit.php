...
class Edit extends BaseStandaloneForm implements ClientArea
{
    ...
    private function addUnlimitedTypeFields()
    {
        ...
        $formData = sl('request')->get('formData');
        if ($this->dataProvider->getValueById('uemail_daily_limitUnlimited') === 'on')
        {
            if ($formData && $formData['uemail_daily_limitUnlimited'] === 'off')
            {
                $this->addFieldAfter('uemail_daily_limitUnlimited', $this->getOptTextField('uemail_daily_limit'));
            }
        }
        else
        {
            if ($formData && $formData['uemail_daily_limitUnlimited'] === 'on')
            {
                return;
            }
            $this->addFieldAfter('uemail_daily_limitUnlimited', $this->getOptTextField('uemail_daily_limit'));
