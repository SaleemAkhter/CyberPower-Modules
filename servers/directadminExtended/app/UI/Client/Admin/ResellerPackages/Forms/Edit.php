...
class Edit extends BaseStandaloneForm implements ClientArea
{
    ...
    public function initContent()
    {
        ...
        $this->addField(new Fields\Hidden('oldname'));
        // $this->addUsage();
        // $this->addBandwidth();
    ...
    private function addUnlimitedTypeFields()
    {
        ...
        $formData = sl('request')->get('formData');
        if ($this->dataProvider->getValueById('nusersUnlimited') === 'on')
        {
            if ($formData && $formData['nusersUnlimited'] === 'off')
            {
                $this->addFieldAfter('nusersUnlimited', $this->getOptTextField('nusers'));
            }
        }
        else
        {
            if ($formData && $formData['nusersUnlimited'] === 'on')
            {
                return;
            }
            $this->addFieldAfter('nusersUnlimited', $this->getOptTextField('nusers'));
    ...
    private function addSwitcherSection()
    {
        ...
        ->addField((new UnlimitedSwitcher('nusersUnlimited'))->setDefaultValue($data['nusersUnlimited'])->addHtmlAttribute('@change', 'changeUnlimitedFields(\'nusers\')')->setTextFieldValue($data['nusers']))
        ->addField( new Fields\Text('ips'))
        ...
        ->addField( new Fields\Switcher('userssh'))
        ->addField( new Fields\Switcher('oversell'))
        ->addField( new Fields\Switcher('cron'))
        ->addField( new Fields\Switcher('sysinfo'))
        ->addField( new Fields\Switcher('login_keys'))
        ->addField( (new Fields\Switcher('dnscontrol')))
        ->addField( (new Fields\Switcher('serverip'))->setDescription('serveripdescription'));
    }
