...
class IpEdit extends ProviderApi
{
    ...
    {
        parent::update();
        // {"reseller":"resone","json":"yes","action":"select","assign":"yes","select0":"193.31.29.61"}:
        $data = [
            'reseller'      => $this->formData['reseller'],
            'select0' => $this->formData['ip'] ,
        ];
        $this->loadAdminApi();
        $response=$this->adminApi->ip->admin_assign($data);

        return (new ResponseTemplates\RawDataJsonResponse())->setMessageAndTranslate('ipAssignedSuccessfully');
    }
