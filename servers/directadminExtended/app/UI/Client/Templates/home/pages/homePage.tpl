...
<div class="lu-tiles lu-row lu-row--eq-height">
    {foreach from=$rawObject->getSection($section) key=setting item=controller}
        <div class="lu-col-xs-6 lu-col-md-20p">
            <a class="lu-tile lu-tile--btn hresonc" href="{$rawObject->getRedirectUrl($controller)}">
                <div class="lu-i-c-6x">
                    {if $controller eq "Applications" }
                        <img src="{$assetsUrl}/img/directadmin/icon-{$controller}.svg" alt="">
                    {else}
                    <img src="{$assetsUrl}/img/directadmin/icon-home.svg#{$controller}" alt="">
