<div class="lu-modal show" id="confirmationModal">
    <div class="lu-modal__dialog">
        <div class="lu-modal__content" id="mgModalContainer">
            ...
            {assign var="addDbUser" value=$rawObject->getForm('settingsForm')}
            <div class="lu-modal__nav">
                <ul class="lu-nav lu-nav--md lu-nav--h lu-nav--tabs">
                    {assign var="sectArrKeys" value=$addDbUser->getSections()|array_keys}
                    {foreach from=$addDbUser->getSections() key=sectionID  item=section }
            ...
            <div class="lu-modal__body">
                <form id="{$addDbUser->getId()}" namespace="{$addDbUser->getNamespace()}" index="{$addDbUser->getIndex()}" mgformtype="{$addDbUser->getFormType()}">
                    {foreach from=$addDbUser->getFields() item=field }
                        {$field->getHtml()}
                    {/foreach}
