mgJsComponentHandler.addDefaultComponent('mg-backup-schedule', {
    template : '#t-mg-backup-schedule',
    props: [
    'component_id',
    'component_namespace',
    'component_index'
    ...
    methods: {
        isActiveTab:function(currentstep){
            if(this.step === currentstep){
                jQuery(".lu-tab-pane").removeClass("is-active");
                jQuery("#contTabstep"+currentstep).addClass("is-active");
                // console.log(jQuery("#contTabstep"+currentstep));
            }
        },
        ...
        initOptionsSelect: function() {
            ...
            $('#skipsuspended').iCheck('destroy');
            $('.check-all-users').iCheck('destroy');

            $('.check-all-users').on('change',function(){
                if(jQuery(this).is(":checked")){
                    jQuery('.user-check').prop('checked','checked');
                    jQuery('.reseller-check').prop('checked','checked');
                }else{
                    jQuery('.user-check').prop('checked','');
                    jQuery('.reseller-check').prop('checked','');
                }
            });

            $('.checkalldataoptions').iCheck('destroy');
            $('.checkalldataoptions').on('change',function(){
                if(jQuery(this).is(":checked")){
                    jQuery('.dataopts').prop('checked','checked');
                }else{
                    jQuery('.dataopts').prop('checked','');
                }
            });
            if(jQuery('.dataopts').not(":checked").length==0){
                $('.checkalldataoptions').prop('checked','checked');
            }else{
                $('.checkalldataoptions').prop('checked','');
            }
            // name="applyto"
        },
        ...
        selectAllUsers:function(){
            console.log("jsjhss");
            if(jQuery(".check-all-users").is(":checked")){
                jQuery('.user-check').iCheck('check');
                jQuery('.reseller-check').iCheck('check');
            }else{
                jQuery('.user-check').iCheck('uncheck');
                jQuery('.reseller-check').iCheck('uncheck');
            }
        },
        selectUser:function (user,reseller){
            if(jQuery('[parentreseller="'+reseller+'"]').not(":checked").length==0){

                jQuery('input.reseller-check[value="'+reseller+'"]').iCheck('check');
            }else{
                jQuery('input.reseller-check[value="'+reseller+'"]').iCheck('uncheck');
            }
            //
        },
        ...
        selectResellerUsers:function(reseller){
            var self=this;
            if(jQuery("input.reseller-check[value='"+reseller+"']").is(":checked")){
