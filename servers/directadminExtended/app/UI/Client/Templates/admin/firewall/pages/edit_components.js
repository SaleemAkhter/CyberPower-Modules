mgJsComponentHandler.addDefaultComponent('mg-firewall-edit', {
    template : '#t-mg-firewall-edit',
    props: [
    'component_id',
    'component_namespace',
    'component_index'
    ...
    methods: {
        ...
        removeSpinner : function(event) {
            if($(event.target)[0].tagName==="IMG"){
                $('[originall-button-content]').html($('[originall-button-content]').attr('originall-button-content'));
                $('[originall-button-content]').removeAttr('originall-button-content');
            }else{
        ...
        showSpinner : function(event) {
            var self = this;
            var spinnerTarget = self.getSpinerTarget(event);
            if (spinnerTarget !==null && (spinnerTarget.length > 0 || $(spinnerTarget).tagName === 'I')) {
                if($(event.target)[0].tagName === 'IMG'){
                    var elWidth = $(event.target).closest('a').width();
                    var spinnerClass = $(event.target).closest('a').hasClass('lu-btn--success') ? 'lu-preloader-success' : ($(event.target).closest('a').hasClass('lu-btn--danger') ? 'lu-preloader-danger' : '');
                    $(event.target).closest('a').attr('originall-button-content', $(event.target).closest('a').html());
                    $(event.target).closest('a').html('<span class="lu-btn__icon lu-temp-button-loader" style="margin: 0 0 0 0 !important; width: ' + elWidth + 'px;"><i class="lu-btn__icon lu-preloader lu-preloader--sm ' + spinnerClass + '"></i></span>');
                }else{
        ...
        getSpinerTarget : function(event) {
            if($(event.target)[0].tagName === 'I'){
                ...
                return $(event.target).find('i').first();
            }else if($(event.target)[0].tagName === 'IMG'){
                return $(event.target);
            } else {
                return null;
            }
        ...
        firewallaction:function(targetId,event,action){
