...
use ModulesGarden\Servers\OvhVpsAndDedicated\App\Models;
use ModulesGarden\Servers\OvhVpsAndDedicated\App\Models\Whmcs\CustomField;
use ModulesGarden\Servers\OvhVpsAndDedicated\App\Models\Whmcs\CustomFieldValue;
...
class CustomFields
{
    ...
    public static function create($productID, $fieldName, $fieldType = "text", $adminOnly = "on", $showorder = "", $description = "")
    {
        $fieldName = explode('|', $fieldName);
        $field     = self::getField($productID, $fieldName[0]);
        if (is_null($field))
        {
            self::createField('product', $productID, implode('|', $fieldName), $fieldType, $adminOnly, $showorder, $description);
        }
    }
    ...
    private static function createField($type, $relID, $fieldName, $fieldType = "text", $adminOnly = "on", $showorder = "", $description = "")
    {
    ...
    public static function removeField($productID, $serviceID, $fieldName)
    {
       $fieldValue = self::getFieldValue($productID, $serviceID, $fieldName);
       if(!empty($fieldValue))
       {
           CustomFieldValue::where('id', $fieldValue->id)->delete();
       }
    }
