...
class EmailTemplate
{
    ...
    {
        return \ModulesGarden\Servers\OvhVpsAndDedicated\Core\Models\Whmcs\EmailTemplate::where('name', $templateName)->first();
    }
    ...
    private function activateTempates($templates)
    {
        foreach ($templates as $template)
        {
            if (!$this->checkEmailTemplate($template['name']))
            {
                ...
                $emailTemplate          = new \ModulesGarden\Servers\OvhVpsAndDedicated\Core\Models\Whmcs\EmailTemplate();
