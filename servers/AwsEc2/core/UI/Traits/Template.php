...
trait Template
{
    ...
    protected function loadTemplateDir()
    {
        ...
        $base   = $isCore = null;
        if (str_contains($class, 'ModulesGarden\Servers\AwsEc2\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\Servers\AwsEc2\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\Servers\AwsEc2\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\Servers\AwsEc2\App\UI\\', '', trim($class, '\\'));
        }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\Servers\AwsEc2\Core\Helper\di($parent, null, true);
                if ($new)
                {
