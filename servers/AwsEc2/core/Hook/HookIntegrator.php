...
class HookIntegrator
{
    ...
    protected function addIntegration($className = null)
    {
        ...
        $integrationClassName = '\ModulesGarden\Servers\AwsEc2\App\Integrations\\' . ($this->isAdmin ? 'Admin' : 'Client') . '\\' . $className;
        if (!class_exists($integrationClassName) || !is_subclass_of($integrationClassName, \ModulesGarden\Servers\AwsEc2\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    {
        if (!is_subclass_of($instance, \ModulesGarden\Servers\AwsEc2\Core\Hook\AbstractHookIntegrationController::class))
        {
            return false;
        }
    ...
    public function loadIntegrationData()
    {
        foreach ($this->integrations as $integration)
        {
            ...
            /** @var
             * $integrationResult \ModulesGarden\Servers\AwsEc2\Core\UI\View
             */
            $integrationResult = call_user_func([di($callbackData[0]), $callbackData[1]]);
            if (!($integrationResult instanceof \ModulesGarden\Servers\AwsEc2\Core\UI\View))
            {
    ...
    public function isIntegrationApplicable($integration = null)
    {
        ...
        if ((!is_subclass_of($integrationCallback[0], \ModulesGarden\Servers\AwsEc2\Core\Http\AbstractController::class)
            && !is_subclass_of($integrationCallback[0], \ModulesGarden\Servers\AwsEc2\Core\Http\AbstractClientController::class))
