...
use ModulesGarden\Servers\AwsEc2\Core\App\Installer\ModuleInstaller;
 ...
 */
class ConfigOptions extends \ModulesGarden\Servers\AwsEc2\Core\App\Controllers\Instances\AddonController
{
    use AppParams;
    ...
    public function execute($params = null)
    {
        ...
        if (($this->getRequestValue('action') === 'module-settings' || ($this->getRequestValue('loadData') && $this->getRequestValue('ajax') == '1')))
        {
            ...
            try
            {
                $this->setAppParam('IntegrationControlerName', \ModulesGarden\Servers\AwsEc2\App\Http\Actions\ConfigOptions::class);
                $this->setAppParam('IntegrationControlerMethod', 'runExecuteProcess');

                $configOptionsController = new \ModulesGarden\Servers\AwsEc2\App\Http\Actions\ConfigOptions();
        ...
        else if ($this->getRequestValue('action') === 'save')
        {
            ...
            return [\ModulesGarden\Servers\AwsEc2\App\Http\Actions\ConfigOptions::class, 'runExecuteProcess'];
        }
    ...
    public function isCorrectServerType()
    {
        try
        {
            if (class_exists('\ModulesGarden\Servers\AwsEc2\App\Http\Actions\MetaData'))
            {
                $metaDataController = new \ModulesGarden\Servers\AwsEc2\App\Http\Actions\MetaData();
    ...
    public function addRequiredCustomFields()
    {
        ...
        $product = new \ModulesGarden\Servers\AwsEc2\Packages\WhmcsService\Product($pid);
