...
class BuildUrl
{
    public static function getUrl($controller = null, $action = null, array $params = [], $isFullUrl = true)
    {
        if (isAdmin())
        {
            $url = 'clientsservices.php?module=' . getModuleName();
        }
        else
        {
            $url = 'clientarea.php?action=productdetails&modop=custom&a=management' ;
        }
    ...
    {
        $modulePath = \ModulesGarden\Servers\AwsEc2\Core\ModuleConstants::getFullPath();
