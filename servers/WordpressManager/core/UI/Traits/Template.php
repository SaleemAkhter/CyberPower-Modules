...
trait Template
{
    ...
    protected function loadTemplateDir()
    {
        ...
        $base   = $isCore = null;
        if (str_contains($class, 'ModulesGarden\WordpressManager\Core\UI\\'))
        {
            $base   = str_replace('ModulesGarden\WordpressManager\Core\UI\\', '', trim($class, '\\'));
            $isCore = true;
        }

        if (str_contains($class, 'ModulesGarden\WordpressManager\App\UI\\'))
        {
            $base = str_replace('ModulesGarden\WordpressManager\App\UI\\', '', trim($class, '\\'));
        }
    ...
    protected function evaluateTemplatePath()
    {
        if (!file_exists($this->templateDir . $this->templateName . '.tpl'))
        {
            ...
            if ($parent)
            {
                $new = \ModulesGarden\WordpressManager\Core\Helper\di($parent, null, true);
                if ($new)
                {
