...
class InstallationDetailsTopNav extends BaseContainer implements ClientArea, AjaxElementInterface
{
    ...
    public function initContent()
    {
        ...
        : BuildUrl::getUrl('home', 'new',['id'=>$request->get('id', '')]);
        ...
        $baseUrl = $request->get('action', false) === 'productdetails' && $request->get('mg-page', false) === 'wordPressManager' ?
        'clientarea.php?action=productdetails&id=' . $request->get('id', '') . '&modop=custom&a=management&mg-page=wordPressManager&mg-action=remoteimport&id='.$request->get('id', '')
        : BuildUrl::getUrl('home', 'remoteimport',['id'=>$request->get('id', '')]);
