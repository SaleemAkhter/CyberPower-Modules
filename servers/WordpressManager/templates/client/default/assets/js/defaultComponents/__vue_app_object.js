...
var mgDefauleVueObject = {
            ...
            methods: {
                ...
                rendKey: function (lenth, avChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789") {
                    if (!lenth) {
                        lenth = 6;
                    }
                    var randKey = "";
                ...
                reloadModalContent: function(event, targetId, namespace, index, params) {
                    ...
                    $.post(self.targetUrl, formData, {}, 'json').done(function (data) {
                        data = data.data;
                        if(data.status === 'success'){
                           self.htmlContent = data.rawData.htmlData;
                ...
                submitForm : function(targetId, event) {
                    ...
                    if(formTargetId){
                        ...
                        self.refreshUrl();
                        if(formTargetId=="installationCreateForm"){

                            jQuery("#installationCreateForm").hide();
                            jQuery("#progress_bar").show();
                            jQuery( "#progress_bar" ).scrollTop( 300 );
                            ProgressBar.start();
                            window.scrollTo(0, 0);
                        }
                        ...
                        .done(function( data ) {
                            ...
                            if(data.status === 'success'){
                                if(formTargetId=="installationCreateForm"){
                                    jQuery("#insurl").attr('href',data.htmlData.site_url).text(data.htmlData.site_url);
                                    jQuery("#adminurl").attr('href',data.htmlData.autologinurl).text(data.htmlData.admin_url);
                                    jQuery("#installationCreateForm").hide();
                                ...
                                self.handleErrorMessage(data);
                                if(formTargetId=="installationCreateForm"){
                                    jQuery("#installationCreateForm").show();
                                    jQuery("#progress_bar").hide();
                                    ProgressBar.end();
                                }
                            }
